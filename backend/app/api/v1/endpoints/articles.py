    ...
    try:
        ...
        try:
            if background_tasks is not None:
                ...
                logger.info(
                    f"📥 Enqueued background translation for article {article_id}"
                )
        except Exception as e:
...
async def update_article(
    ...
    try:
        ...
        if article["author_id"] != current_user["sub"] and current_user["role"] not in [
            "admin",
            "moderator",
        ]:
...
async def enhance_article_with_ai(
    ...
    try:
        ...
        category = await gemini_service.classify_category(
            article["title"], article["content"]
        )
...
async def publish_article(
    ...
    try:
        ...
        if article["status"] != "approved":
            raise HTTPException(
                status_code=400, detail="Article must be approved first"
            )
        ...
        # Update article
        update_data = {"publish_time": publish_time, "updated_at": datetime.utcnow()}
...
async def _translate_and_save(article_id: str, db):
    ...
    try:
        if db is None:
            logger.warning(
                "⚠️ DB instance is None; cannot perform background translation"
            )
            return
        ...
        try:
            existing = await db.english_trans.find_one(
                {"article_id": str(article.get("_id"))}
            )
            if existing:
                logger.info(
                    f"ℹ️ english_trans already exists for {article_id}, skipping background translate"
                )
                return
        except Exception:
        ...
        try:
            ...
            if getattr(google_service, "translate_client", None):
                try:
                    title_tr = await google_service.translate_text(
                        article.get("title", ""),
                        target_language="en",
                        source_language=source_lang,
                    )
                    content_tr = await google_service.translate_text(
                        article.get("content", ""),
                        target_language="en",
                        source_language=source_lang,
                    )
                    ...
                    if article.get("excerpt"):
                        ex_tr = await google_service.translate_text(
                            article.get("excerpt", ""),
                            target_language="en",
                            source_language=source_lang,
                        )
                ...
                try:
                    ...
                    translated_text = await gemini_service.generate_text(
            ...
            if not translated_content:
                logger.warning(
                    f"⚠️ No translated content produced for article {article_id}"
                )
                return
            ...
            await db.english_trans.update_one(
                {"article_id": tr_doc["article_id"]}, {"$set": tr_doc}, upsert=True
            )
            logger.info(
                f"✅ Background translated and saved english_trans for article {article_id}"
            )
...
async def find_similar_articles(
    article_id: str, limit: int = Query(5, ge=1, le=20), db=Depends(get_database)
):
...
async def get_public_articles(
    ...
    try:
        ...
        # Search support: if `search` provided, do a case-insensitive regex match
        # against title, excerpt, or content. This avoids requiring a text index.
        if search:
            # escape user input minimally to avoid accidental regex injection
            # but keep it simple (treat search as literal substring)
            import re
        ...
        for article in articles:
            ...
            mapped = {
                ...
                "comment_count": article.get(
                    "commentsCount", article.get("comment_count", 0)
                ),
        ...
        logger.info(
            f"✅ Public articles: Found {len(mapped_articles)} published articles"
        )
...
async def get_trending_articles(
    limit: int = Query(5, ge=1, le=20), db=Depends(get_database)
):
    """
...
async def get_public_categories(db=Depends(get_database)):
    ...
    try:
        ...
        categories = [{"name": cat["_id"], "count": cat["count"]} for cat in result]
...
async def get_public_article_by_id(article_id: str, db=Depends(get_database)):
    try:
        ...
        if not article:
            article = await db.articles.find_one(
                {"_id": article_id, "status": "published"}
            )
        # Try ObjectId
        if not article:
            try:
                article = await db.articles.find_one(
                    {"_id": ObjectId(article_id), "status": "published"}
                )
            except Exception:
                pass
        if not article:
        ...
        # Increment view count
        await db.articles.update_one(
            {"_id": article["_id"]}, {"$inc": {"view_count": 1}}
        )
        ...
        mapped = {
            ...
            "created_at": parse_dt(
                article.get("created_at") or article.get("createdAt")
            ),
            "updated_at": parse_dt(
                article.get("updated_at") or article.get("updatedAt")
            ),
        }
        logger.info(
            f"✅ Public article: {article_id} - {mapped['title'][:50]} - Views: {mapped['view_count']}"
        )
...
@router.post(
    "/public/articles/{article_id}/translate/{lang}", response_model=TranslationResponse
)
async def public_translate_article(
    article_id: str, lang: str, db=Depends(get_database)
):
    ...
    try:
        ...
        try:
            source_lang = (
                await google_service.detect_language(article.get("title", ""))
                if getattr(google_service, "translate_client", None)
                else "vi"
            )
        except Exception:
            ...
            if lang == "en":
                try:
                    ...
                    try:
                        ...
                        tr_doc["content"] = clean_translated_text(
                            tr_doc.get("content", "")
                        )
                    except Exception:
                        pass
                    await db.english_trans.update_one(
                        {"article_id": article_id_str}, {"$set": tr_doc}, upsert=True
                    )
                except Exception as e:
                    logger.warning(f"⚠️ Failed to persist english_trans document: {e}")
            else:
                await db.articles.update_one(
                    {"_id": article["_id"]}, {"$set": {f"translations.{lang}": tr_doc}}
                )
